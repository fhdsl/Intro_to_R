...
All function arguments are stored in the **local environment**. Any additional variables defined within `{ }` is also stored in the local environment. The overall environment of the program is called the **global environment** and can be also accessed within { }. The local environment is erased after `}` is reached.
:::
