...
## Comments on `filter()` and `select()`

`metadata_filtered = filter(metadata, OncotreeLineage == "Breast")`:

. . .

The second argument is strange: the expression we give it looks like a logical indexing vector built from a comparison operator.
...
Rather, `OncotreeLineage` is a column from `metadata`, and we are referring to it as a **data variable**.
