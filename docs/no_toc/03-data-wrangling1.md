...
Suppose you are given this vector:
...
Here are two scenarios of subsetting:
...
2.  **Implicit subsetting**: Suppose someone approaches you a length 10 vector of people's ages, and say that they want to subset to elements greater than 50.

We already know how to subset explicitly:
...
But for implicit subsetting, we don't know off the top of our head which elements have a value greater than 50. If this vector was really big, it wouldn't make sense for us to manually figure this out. Luckily, we already have the tools we need to subset vectors implicitly.

Recall that our comparison operators can be applied for vectors, such as:
...
This vector of `TRUE` and `FALSE` values can be used as a logical indexing vector to help us subset implicitly!
...
Let's subset `staff` so it doesn't have "chris" in it.
...
Then, use the resulting value as a logical indexing vector:


``` r
answer = staff[staff != "chris"]
answer
```
...
We stored the subsetted result as a variable `answer`.

Let's take this idea of implicit subsetting to Dataframes.
...
When computing the mean value of the `Age` column from `breast_metadata`, we add an additional option of `na.rm = TRUE` because some of the column contain missing values. This will remove the missing values before computing the mean value.
