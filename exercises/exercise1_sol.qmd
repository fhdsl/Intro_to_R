...
## Part 3: Installing a R package

To prepare for next week's class, we ask you to install a R package we will be using from next class forward. R packages contain useful functions and data for a category of tasks. For instance, the package `ggplot2` contains functions useful for data visualization.

In your R console, type in `install.packages("tidyverse")` and hit enter. Your R console will be busy for a while installing the package. Once it is done, in your console again, type in `library(tidyverse)` and hit enter. The command `library()` loads the package into your environment for use. It will print out the status of attaching core tidyverse packages and should be ready for use. We will talk about this more next week.

You only need to install a package once on the computer you are working on. However, you will need to load the package into your environment every time you want to access it. Often, in a R script, authors load all the libraries they need for the analysis at the top of the script.
